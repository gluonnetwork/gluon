...
\textbf{Proof.} \emph{Trade} entries for a given execution $ex$ should have exactly four entries and their quantities should add up to zero.
\begin{equation}
    ex.length = 4
\end{equation}
\begin{equation}
...
Similarly, up to four \emph{Fee} entries $f_i$ may exist per execution.
\begin{equation}
    ex.length = 4 \lor ex.length = 2
\end{equation}
\begin{equation}
f_1(A_1,Z_1).Qty = -f_2(A_2,Z_1).Qty 
\end{equation}
\begin{equation}
f_3(A_1,Z_1).Qty = -f_4(A_2,Z_1).Qty 
\end{equation}
...
\begin{equation}
    v_1 \rightarrow v_2 \rightarrow v_3 \implies v_1 \rightarrow v_3
\end{equation}
...
\rowcolor{yellow!50} Suppress  &   \ref{fp:bcf}  & \ref{gp:exit} & \ref{fp:eif}   &  No effect\footnotemark & No effect\footnotemark \\
\rowcolor{yellow!50} Reorder  & \ref{fp:cff}   & \ref{fp:cff} & \ref{fp:eif}   & \ref{fp:ptp}  & \ref{fp:ptp} \\
\hline
\end{tabular}
\end{table}
\addtocounter{footnote}{-2}
 \stepcounter{footnote}\footnotetext{Suppressed \emph{Trade} entries are equivalent to cancelled orders}
\stepcounter{footnote}\footnotetext{Suppressed \emph{Fee} entries are equivalent to free trades}
\newpage
\centering
